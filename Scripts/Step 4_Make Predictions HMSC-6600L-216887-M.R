...
predict_mean_occurrence <- function(coords, covs, model.object) {
  ...
  # Calculate mean and upper and lower 90% quantiles
  EpredY = Reduce("+", predY) / length(predY)
  quantiles_result <- calculate_quantiles(predY, 0.025, 0.975)
  lower_quantile <- t(quantiles_result[,,1])
  colnames(lower_quantile) = paste0(colnames(EpredY),"_LCI")
  upper_quantile <- t(quantiles_result[,,2])
  colnames(upper_quantile) = paste0(colnames(EpredY),"_UCI")
  prediction.out = cbind(xy.grid_chunk, EpredY, lower_quantile, upper_quantile)
}
...
for(i in 1:length(chunks)){
  ...
  print(paste0("Chunk number ", i, " done in ", Sys.time() - start.time))
} # started at 9.05pm on 14/06/2024 - 

#30 min per 10000 chunk
# 4.5 hours per 100,000 chunk
...
predict_mean_occurrence <- function(coords, covs, model.object) {
  predict.gradient =Hmsc::prepareGradient(model.object, XDataNew = covs, 
                                          sDataNew = list(sample=coords))
  predY = predict(model.object, Gradient = predict.gradient, 
                  expected = TRUE, nParallel = 8, predictEtaMean = TRUE)
  # Calculate mean and upper and lower 90% quantiles
  EpredY = Reduce("+", predY) / length(predY)
  quantiles_result <- calculate_quantiles(predY, 0.025, 0.975)
  lower_quantile <- t(quantiles_result[,,1])
...
saveRDS(pred.list, file = "Data_Clean/hmsc_postfire_spatialpredictions_gippsland.RDS")
...
for(i in 1:length(chunks)){
  ...
  predictions = rbind(predictions, pred.out)
  print(paste0("Chunk number ", i, " done at ", Sys.time()))
} # started at 9.48am on 27/03/2024 - 
